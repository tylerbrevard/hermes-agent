...
def _format_timestamp(ts: Union[int, float, str, None]) -> str:
    """Unix timestamp -> readable date; valid ISO dates use the same format; "unknown" for None."""
    if ts is None:
        return "unknown"
    try:
        if isinstance(ts, str):
            try:
                ...
                value = ts.strip()
                # Support ISO date-only and datetime strings; preserve non-date legacy values.
                try:
        ...
        return str(ts)
    return _quiet(lambda: safe_strftime(datetime.fromtimestamp(numeric_ts), "%B %d, %Y at %I:%M %p"), str(ts),
