...
_EXCLUDED_DIRS = {
    ...
    # Browser Use profile names are handled root-scoped by _in_excluded_root_dir.
    "browser_profiles", "browser-profiles", "browser-profile",
...
# Browser Use creates these credential-bearing Chromium profile trees directly under
# each Hermes home. Unlike the general exclusions, same-named user directories deeper
# in a home are durable data and must remain in backups.
_EXCLUDED_BROWSER_PROFILE_DIRS = {"browser_profiles", "browser-profiles", "browser-profile"}
...
def _in_excluded_root_dir(rel_path: Path) -> bool:
    ...
    if not parts:
        return False
    if parts[0] in _EXCLUDED_ROOT_DIRS or parts[0] in _EXCLUDED_BROWSER_PROFILE_DIRS:
        return True
    if len(parts) >= 3 and parts[0] == "profiles" and parts[2] in _EXCLUDED_BROWSER_PROFILE_DIRS:
        return True
...
def _should_exclude(rel_path: Path) -> bool:
    ...
    if any(
        p in _EXCLUDED_DIRS
        and p not in _EXCLUDED_BROWSER_PROFILE_DIRS
        and (p != "hermes-agent" or p == parts[0])
        for p in parts
    ):
        return True
    name = rel_path.name
    ...
    for dirpath, dirnames, filenames in os.walk(hermes_root, followlinks=False):
        ...
        kept = [
            ...
            if (
                ...
                or d in _EXCLUDED_BROWSER_PROFILE_DIRS
